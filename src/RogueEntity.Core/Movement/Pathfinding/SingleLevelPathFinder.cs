...
{
    public class SingleLevelPathFinder : AStarGridBase<Position2D, IMovementMode>
    {
        static readonly ILogger Logger = SLog.ForContext<SingleLevelPathFinder>();
        ...
        public SingleLevelPathFinder(IBoundedDataViewPool<AStarNode> pool,
        ...
        protected override bool EdgeCostInformation(in Position2D sourceNode, in Direction d, float sourceNodeCost, out float totalPathCost, out IMovementMode movementMode)
        {
            ...
            {
                if (!m.Directions.TryGet(targetPos.GridX, targetPos.GridX, out var dir) ||
                    !dir.IsMovementAllowed(d))
                {
                    continue;
                }
