...
public class StackedGridMapDataContext<TItemId> : IMapDataContext<TItemId>
    where TItemId : struct, IEntityKey
{
    ...
    public StackedGridMapDataContext(MapLayer mapLayer,
        ...
        where TPosition : struct, IPosition<TPosition>
    {
        var pos = position.ToGridXY();
        var z = position.GridZ;
        var cpos = EntityGridPosition.From(position);
        buffer = BufferList.PrepareBuffer(buffer);
        using var indexBuffer = BufferListPool<FreeListIndex>.GetPooled();
        if (layers.TryGetValue(z, out var layer))
        {
        ...
        where TPosition : struct, IPosition<TPosition>
    {
        ...
        gridIndex.DetailedPositions[idx] = EntityGridPosition.From(desiredPosition);
        return true;
    }
    ...
    class LayerData
    {
        ...
        public readonly Dictionary<FreeListIndex, EntityGridPosition> DetailedPositions;

        public LayerData(GridIndex2D<TItemId> layerData)
        {
            this.Data = layerData;
            this.DetailedPositions = new Dictionary<FreeListIndex, EntityGridPosition>();
        }
