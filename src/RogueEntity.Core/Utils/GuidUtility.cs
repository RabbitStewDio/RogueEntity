...
namespace RogueEntity.Core.Utils
{
    ...
    public static class GuidUtility
    {
        ...
        public static Guid Create(Guid namespaceId, byte[] nameBytes, int version)
        {
            ...
            var tmp = ArrayPool<byte>.Shared.Rent(16 + nameBytes.Length);
            Array.Copy(namespaceBytes, tmp, namespaceBytes.Length);
            Array.Copy(nameBytes, 0, tmp, namespaceBytes.Length, nameBytes.Length);
            
            // compute the hash of the namespace ID concatenated with the name (step 4)
            byte[] hash;
            using (var algorithm = version == 3 ? (HashAlgorithm)MD5.Create() : SHA1.Create())
            {
                hash = algorithm.ComputeHash(tmp);
            }
