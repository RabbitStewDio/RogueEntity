...
namespace RogueEntity.DndKit.DiceNotation
{
	...
	public class Parser : IParser
	{
		...
		static readonly Dictionary<char, int> OperatorPrecedence = new Dictionary<char, int>()
		{
			['('] = 1,
			['+'] = 2,
			['-'] = 2,
        ...
        static List<string> ToPostfix(string infix)
		{
			...
			while (charIndex < infix.Length)
			{
				...
				else // Separate so we can increment charIndex differently
				{
					...
					else if (OperatorPrecedence.TryGetValue(infix[charIndex], out var currentPrecedence))
					{
						while (operators.Count > 0 && OperatorPrecedence[operators.Peek()] >= currentPrecedence)
						{
