...
namespace DefaultNamespace
{
    ...
    public static class ToolPathResolver
    {
        ...
        {
            var x2 = ExecutingAssemblyDirectory != null
                ? Path.Combine(ExecutingAssemblyDirectory, packageIds[0])
                : null;
            var y = NuGetAssetsConfigFile != null
                ? NuGetPackageResolver.GetLocalInstalledPackage(packageIds[0], NuGetAssetsConfigFile, version)?.Directory
                : null;
            var x1 = NuGetPackagesConfigFile != null
                ? NuGetPackageResolver.GetLocalInstalledPackage(packageIds[0], NuGetPackagesConfigFile, version)?.Directory
                : null;
            var y2 = PaketPackagesConfigFile != null
                ? PaketPackageResolver.GetLocalInstalledPackageDirectory(packageIds[0], PaketPackagesConfigFile)
                : null;
